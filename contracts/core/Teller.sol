...
contract Teller {
    ...
    function init(address _geo, address _zone) external onlyWhenNotInited {
        zone = IZone(_zone);
    ...
    function getReferrer() external view returns (address ref, uint256 refFee) {
    ...
    function toBytes7(bytes memory _bytes, uint256 _start)
        private
        pure
        returns (bytes7)
    {
    ...
    function removeTellerByZone() external onlyByZoneContract {
        _remove();
        // audit feedback
        // _removeComments();
    }
    ...
    function addTeller(
        ...
        bytes1 _settings,
        address _referrer,
        uint256 _refFee, // referral fees x 10 (exemple, for 21.3 % -> 213) Max is 33.3%. The fees its taken from the harbeger taxes
        ...
        onlyWhenHasNoTeller
    {
        ...
        if (_settings & isBuyerBitMask != 0) {
            // buyer bit is set => teller is a "buyer"
            require(
                _buyRate >= -999 && _buyRate <= 9999,
