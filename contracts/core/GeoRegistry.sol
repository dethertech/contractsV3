...
contract GeoRegistry {
    ...
    function validGeohashChars(bytes memory _bytes) public view returns (bool) {
        require(_bytes.length > 0, "_bytes geohash chars is empty array");

        for (uint256 i = 0; i < _bytes.length; i += 1) {
    ...
    function validGeohashChars12(bytes12 _bytes) public view returns (bool) {
    ...
    function updateLevel2batch(
        bytes2 _zoneCode,
        bytes3[] memory _letters,
        bytes4[] memory _subLetters
    ) public {
        require(!zoneIsEnabled[_zoneCode], "zone must not be enabled");
    ...
    function endInit(bytes2 _zoneCode) external {
        require(!zoneIsEnabled[_zoneCode], "zone must not be enabled");
