...
contract ZoneFactory is IERC223ReceivingContract, EIP1167CloneFactory {
    ...
    modifier onlyZone() {
        ...
        require(
            geohashToZone[zoneToGeohash[msg.sender]] == msg.sender,
    ...
    constructor(
        ...
        require(
            _zoneImplementation != address(0),
            "zoneImplementation address cannot be 0x0"
        );
        require(
            _tellerImplementation != address(0),
     ...
     */
    function fillCurrentZoneBidder(address bidder) public onlyZone {
        if (activeBidderToZone[bidder] == address(0)) {
            require(
                zoneToAuctionBidders[msg.sender].length <= 21,
                "there is already 21 bidder on this auction"
            ); // no more than 21 bidder at the same time
     ...
     */
    function removeCurrentZoneBidders() public onlyZone {
     ...
     */
    function removeActiveBidder(address activeBidder) public onlyZone {
    ...
    function emitReleaseZone(bytes6 zoneFrom, address sender) public onlyZone {
    ...
    function tokenFallback(
        ...
        require(
            geo.zoneInsideBiggerZone(country, bytes4(geohash)),
            "zone is not inside country"
        );
        require(geohashToZone[geohash] == address(0), "zone already exists");
        require(
            ownerToZone[sender] == address(0),
            "address already own a zone"
        );
        require(
            activeBidderToZone[sender] == address(0),
            "sender is currently involved in an auction (zoneFactory)"
        );
