...
const toBitMap = (chars) => {
  ...
  chars.forEach((char) => {
    switch (char) {
      case "v":
        res = res.or("2147483648");
        break;
      case "y":
        res = res.or("1073741824");
        break;
      case "z":
        res = res.or("536870912");
        break;
      case "b":
        res = res.or("268435456");
        break;
      case "c":
        res = res.or("134217728");
        break;
      case "f":
        res = res.or("67108864");
        break;
      ...
      case "h":
        ...
        break;
      default:
        throw new Error(`unknown geohash char ${char}`);
    }
  });
...
const addCountry = async (
  ...
  const countryMap = Object.keys(countryFile).reduce((memo, level0char) => {
    // console.log("countryMap ===> ", countryFile, memo, level0char);
    Object.keys(countryFile[level0char]).forEach((level1char) => {
      Object.keys(countryFile[level0char][level1char]).forEach((level2char) => {
        const level4chars = Object.keys(
          countryFile[level0char][level1char][level2char]
        );
