...
contract("GeoRegistry", () => {
  ...
  after(() => {
    ...
    results
      .sort((a, b) => b.countryGasCost - a.countryGasCost)
      .forEach((item) => {
        console.log(
          `country:${item.countryCode} gas:${addNumberDots(
            item.countryGasCost
          )} txs:${item.txCount} time:${
            item.time
          } mostExpensiveTrx:${addNumberDots(item.mostExpensiveTrxGasCost)}`
        );
      });
  });
    ...
    ["NL"].forEach((countryCode) => {
      ...
      it(`successfully adds country ${countryCode}`, async () => {
        ...
        for (const key in countryMap) {
          const onchainContent = await geoRegistryContract.level_2(
          ...
          assert.deepStrictEqual(
            onchainContent,
            countryMap[key],
            `content for key ${key} does not match expected`
          );
        }
      });
    });
  ...
  it("zoneInsideBiggerZone returns correct result", async () => {
    ...
    const countryFile = require(path.join(
      __dirname,
      "..",
      "data",
      "trees_countries",
    ...
    assert.equal(
      await geoRegistryContract.zoneInsideBiggerZone(
        countryCodeBytes,
        // web3.utils.asciiToHex("kr0ttse")
        web3.utils.asciiToHex("kr0t")
      ),
      true,
      "should be inside"
    );
    ...
    assert.equal(
      await geoRegistryContract.zoneInsideBiggerZone(
        countryCodeBytes,
        // web3.utils.asciiToHex("krrrrrr")
        web3.utils.asciiToHex("krrr")
      ),
      false,
    ...
    assert.equal(
      await geoRegistryContract.zoneInsideBiggerZone(
        countryCodeBytes,
        // web3.utils.asciiToHex("kqgtg75")
        web3.utils.asciiToHex("kqgt")
      ),
      false,
      "should not be inside"
    );
  ...
  it("impossible to add new geohash to already filled up country", async () => {
    ...
    try {
      ...
      if (!err.message.includes("zone must not be enabled")) throw err;
      return;
    }
